...
class TwilioSenderTest < ActiveSupport::TestCase
  setup do
    ...
    @body = 'Test this SMS'
    # AKA stub_chain
    TwilioSender.stubs(twilio_client: stub(account: stub(messages: stub())))
  end

  test 'adds +1 onto numbers and sends sms' do
    TwilioSender.twilio_client.account.messages.expects(:create).with(from: "+1#{@from}", to: "+1#{@to}", body: @body)
